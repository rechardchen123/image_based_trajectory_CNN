...
def data_compensation_algorithm_static(MMSI_list,
    ...
    return MMSI_list, Longitude_list, Latitude_list, Day_list


def data_compensation_algorithm_movement(MMSI_list, Longitude_list, Latitude_list,
                                         Day_list, number_of_compensation, current_position):
...
def save_data_into_file(MMSI_list, Longitude_list, Latitude_list, Day_list):
    ...
    data.to_csv('/home/ucesxc0/Scratch/output/ais_trajectory_compensation_mpi/result/%d-%d.csv' % (name_mmsi, name_day),
                index=False)
...
file_address = glob.glob('/home/ucesxc0/Scratch/output/ais_trajectory_compensation_mpi/AIS-data-after-day-split/*.csv')
            ...
            if delta_time[i] > threshold_time:
                for j in range(1, len(delta_speed) - 1):
                    if delta_speed[j] >= 0 and delta_speed[j] < 2.5:
                        ...
                        current_position = j
                        new_MMSI_list, new_Longitude_list, new_Latitude_list, new_Day_list = \
