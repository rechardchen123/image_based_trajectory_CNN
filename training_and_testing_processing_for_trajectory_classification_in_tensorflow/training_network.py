...
def read_and_decode(record):
    ...
    parsed = tf.parse_single_example(
        record, features=save_image_label_dict)
...
with tf.name_scope('input_layer'):
    ...
    train_logit = build_network.convolution_layer(image_tensor, BATCH_SIZE, N_CLASSES)
    ...
    with tf.Session() as session:
        ...
        try:
            ...
            #                                            label3_tensor:label3})
                tra_loss,tra_acc = session.run([train_logit,train_evaluation],
                                           feed_dict={  image_tensor:image,
                                                        label1_tensor:label1,
                                                        label2_tensor:label2,
                                                        label3_tensor:label3})
